...
export default function Cart({ items }: CartProps) {
  ...
  return (
    <Wrapper>
      ...
      <Skeleton paragraph={false} title={{ width: 128 }} loading={!items || items.length === 0}>
        <Space>
          <Title type="secondary" level={4}>
            Total:{' '}
          </Title>
          <Pricing amount={total} currency={items && items} hasLogo size="large" />
        </Space>
      </Skeleton>
      <Divider />
      <Items>
        ...
        {items &&
          items.length >= 0 &&
          items.map((item, index) => <LineItem key={index} data={item} />)}
      </Items>
    </Wrapper>
  );
}
